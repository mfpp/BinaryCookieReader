...
#                                                                               #
# Usage: python BinaryCookieReader.py <Cookies.binarycookies>                   #
#                                                                               #
# Safari browser and iOS applications store the persistent cookies in a binary  #
# file names Cookies.binarycookies. The BinaryCookieReader is used to dump all  #
# the cookies from the binary Cookies.binarycookies file.                       #
#                                                                               #
#################################################################################
...
if str(file_header)!='cook':
	sys.exit(" error: not a Cookies.binarycookies file")
...
for page in pages:
	...
	for offset in cookie_offsets:
		...
		flags=unpack('<i',cookie.read(4))[0]                #Cookie flags:  1=Secure, 4=HttpOnly, 5=Secure and HttpOnly
		cookie_flags=''
		if flags==0:
			cookie_flags=''
		elif flags==1:
        ...
        index += 1
        print "[%d] Set-Cookie: %s=%s; domain=%s; path=%s; expires=%s%s\n" %(index, name, value, url, path, expiry_date, cookie_flags)
